...
public:	
	...
	//================end
	H3DVec4 QuatMultiply(const H3DVec4& input)const
	{
		H3DVec4 quat;
		quat.Set(w*input.x +x*input.w +y*input.z -z*input.y,
			w*input.y + y*input.w + z*input.x - x*input.z,
			w*input.z + z*input.w + x*input.y - y*input.x,
			w*input.w - x*input.x - y*input.y - z*input.z );
		float l = static_cast<float>(sqrt(quat.x*quat.x+quat.y*quat.y+quat.z*quat.z+quat.w*quat.w));
	...
	H3DVec3	QuatMultiplyVec3( const H3DVec3 &a)
	{
		H3DVec4 qs(-x, -y, -z, w);
		H3DVec4 p(a.x,a.y,a.z,0);
		H3DVec4 r = ((*this).QuatMultiply(p)).QuatMultiply(qs);
		return H3DVec3(r.x, r.y, r.z);
	}
